...
# Based on timing_wrapper.sh from QIIME-Scaling by Jose Navas Molina

# Check if the user has provided a command to execute
if [[ $# -le 1 ]]; then
	echo "USAGE: ./time.sh output_filepath command_to_benchmark [command args]"
	echo "For example: ./time.sh results.txt python test.py --someparam 3"
	exit 1
fi
...
# If the output file exists, rather than overwriting, rename it with date + time information
if [ -f ${output_fp} ]; then
